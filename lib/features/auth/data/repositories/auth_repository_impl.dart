...
class AuthRepositoryImpl implements AuthRepository {
  ...
  // Email/password authentication methods (not used in role-based auth)
  @override
  ...
  Future<Either<Failure, bool>> isLoggedIn() async {
    try {
      ...
      return currentRole.fold(
        (failure) => const Right(false),
        (role) => Right(role != null),
      );
    } catch (e) {
      return Left(UnknownFailure('Login check failed: $e'));
    }
  }

  @override
  Future<Either<Failure, String?>> getCurrentUserEmail() async {
    return const Left(ValidationFailure('Email not supported in role-based auth'));
  }
}
